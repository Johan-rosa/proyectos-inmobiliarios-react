...
export default function PaymentSchedule({
  ...
  const generatePaymentSchedule = () => {
    ...
    const ordinaryAmount = (totalToPay - totalExtra) / newPayments.length
    ...
    const handleDateChange = (id: number, newDate: Date) => {
      const updatedPayments = payments.map((payment) => (payment.id === id ? { ...payment, date: newDate } : payment))
      setPayments(updatedPayments)
    }
  ...
  const handleExtraChange = (id: number, value: string) => {
    ...
    const ordinaryAmount = (totalToPay - totalExtra) / updatedPayments.length
    ...
    // Calculate totals
    const totalOrdinary = payments.reduce((sum, payment) => sum + payment.ordinary, 0)
    const totalExtra = payments.reduce((sum, payment) => sum + payment.extra, 0)
