...
function PaymentBuilderInputs({ values, exportValues }: PaymentBuilderInputsProps) {
  ...
  const debouncedExport = useCallback(
    (newValues: PaymentPlan) => {
      // Only export if values have actually changed
      if (JSON.stringify(newValues) !== JSON.stringify(values)) {
        exportValues(newValues)
      }
    },
    [exportValues, values],
  )
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      debouncedExport(formInputs)
    }, 300) // 300ms debounce

    return () => clearTimeout(timer)
  }, [formInputs, debouncedExport])
  ...
  const onReservationChange = (value: string) => {
    const reservation = Math.min(Number.parseFloat(value), formInputs.price)
  ...
  const onSignatureChange = (value: string) => {
    const signature = Math.min(Number.parseFloat(value), formInputs.price)
  ...
  const onDuringConstructionChange = (value: string) => {
    const duringConstruction = Math.min(Number.parseFloat(value), formInputs.price)
  ...
  const onDuringConstructionPercentChange = (value: string) => {
    const duringConstructionPercent = Math.min(Number.parseFloat(value), 100)
  ...
  const onReservationSignaturePercentChange = (value: string) => {
    const reservationSignatuerPercent = Math.min(Number.parseFloat(value), 100)

    if (!isNaN(reservationSignatuerPercent) && formInputs.price > 0) {
      setFormInputs((prev) => {
        const calculatedReservationPercent =
          prev.reservationPercent >= reservationSignatuerPercent ? reservationSignatuerPercent : prev.reservationPercent
    ...
    <div>
      ...
      <div className="mb-4">
        ...
        <Select
          ...
          onValueChange={(value) => setFormInputs({ ...formInputs, frequency: value })}
        >
